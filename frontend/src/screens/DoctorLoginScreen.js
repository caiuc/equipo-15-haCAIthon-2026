...
export default function DoctorLoginScreen({ navigation }) {
  ...
  async function handleLogin() {
    ...
    setSubmitting(true);
    try {
      await loginDoctor(email.trim().toLowerCase(), password);
    } catch (err) {
  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <Text style={styles.title}>Sanito · Médicos</Text>
      <Text style={styles.subtitle}>Inicia sesión para ver tu agenda</Text>
      ...
      <TouchableOpacity onPress={() => navigation.navigate('Login')}>
        <Text style={styles.link}>¿Eres paciente? Inicia sesión aquí</Text>
      </TouchableOpacity>
    </KeyboardAvoidingView>
  );
}
